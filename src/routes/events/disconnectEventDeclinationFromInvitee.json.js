...
export const post = async ({ request }) => {
	...
	try {
		const query = gql`
			mutation DisconnectEventDeclinationFromInvitee($inviteeId: ID!, $eventId: ID!) {
				invitee: updateInvitee(where: { id: $inviteeId }, data: { declinedEvents: { disconnect: { id: $eventId } } }) {
					id
					declinedEvents {
						id
					}
